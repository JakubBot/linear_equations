...
def solve_jacobi(
    A: np.ndarray, b: np.ndarray, precision: float = 1e-9, max_error: float = 1e9
) -> OutputSolution:
    ...
    while errors[-1] > precision:
        ...
        for i in range(n):
            sum_L_U = sum(
                A[i][j] * x[j][0] for j in range(n) if i != j
            )  # K = (L + U) * x
...
def solve_gauss_seidel_optimized(
    A: np.ndarray, b: np.ndarray, precision: float = 1e-9, max_error: float = 1e9
) -> OutputSolution:
...
def solve_l_u_optimized(
    A: np.ndarray, b: np.ndarray, precision: float = 1e-9
) -> OutputSolution:
    ...
    return OutputSolution(
        x=x.reshape(-1, 1),  # dopasowujemy do poprzedniego interfejsu
        iterations=0,
        errors=[norm],
        does_converge=does_converge,
        time=elapsed_time,
...
def solve_l_u(A: np.ndarray, b: np.ndarray, precision: float = 1e-9) -> OutputSolution:
    ...
    return OutputSolution(
        x=x, iterations=0, errors=[norm], does_converge=does_converge, time=elapsed_time
    )
